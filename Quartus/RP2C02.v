...
output [7:0]DBIN,   // PPU Register Open Bus
...
always @(posedge Clk) begin
        ...
        RnWR <= RnW;
        nDBER <= nDBE;
        W0   <= ~ADR[2] & ~ADR[1] & ~ADR[0] & ~RnWR & ~nDBER; //  replace W1 and W0 with places for replacing PPUMASK and PPUCTRL RGB PPU RC2C05-01
        W1   <= ~ADR[2] & ~ADR[1] &  ADR[0] & ~RnWR & ~nDBER; //  replace W1 and W0 with places for replacing PPUMASK and PPUCTRL RGB PPU RC2C05-01
...
input CLIP_B,       // Blanking the left column of 8 screen dots for background
input MODE,         // PAL/NTSC mode
// Outputs
...
assign CLPO = ~CLIPOR;
assign EMPH[2:0] = MODE ? {W1R[7], EMP_R, EMP_G} : {W1R[7], EMP_G, EMP_R}; // For PAL Red/green color emphasis swapped.
// Logics
always @(posedge Clk) begin
...
//PAR COUNTERS
//                  Clk   F2   DIR              C_IN        Reset   LOAD    STEP   DATA    CNT_OUT     C_OUT
COUNTER THCNT[4:0] (Clk, PCLK, 1'b1, {THOCout[3:0], TH_IN}, 1'b0, THLOAD, THSTEP, TH[4:0], THO[4:0], THOCout[4:0]); // TH  COUNTER
COUNTER TVCNT[4:0] (Clk, PCLK, 1'b1, {TVOCout[3:0], TV_IN}, ZTV,  TVLOAD, TVSTEP, TV[4:0], TVO[4:0], TVOCout[4:0]); // TV  COUNTER
COUNTER NTHCNT     (Clk, PCLK, 1'b1,                NTH_IN, 1'b0, THLOAD, THSTEP, NTH,     NTHDO,    NTHC);         // NTH COUNTER
COUNTER NTVCNT     (Clk, PCLK, 1'b1,                NTV_IN, 1'b0, TVLOAD, TVSTEP, NTV,     NTVDO,    NTVC);         // NTV COUNTER
COUNTER FVCNT[2:0] (Clk, PCLK, 1'b1, {FVOCout[1:0], FV_IN}, 1'b0, TVLOAD, TVSTEP, FV[2:0], FVO[2:0], FVOCout[2:0]); // FV  COUNTER
...
output reg R2DB6  // Spritehit flag reading
);
// Variables
reg [4:0]ZCOLN;
reg [4:0]THO_LATCH;
